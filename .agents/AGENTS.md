...
64. **Lógica de Alarmes Adiados (Snoozed) vs Perdidos**: Para evitar que alarmes com soneca (snooze) ativa sejam considerados perdidos pelas seções e resumos do Dashboard, a verificação de perda deve considerar a duração do snooze (`snoozeMin`) somada à janela de tolerância de 10 minutos (limite = `alarmTime + snoozeMin + 10`). O alarme permanece no estado "Pendente" até que essa hora limite seja ultrapassada.
65. **FABs Arrastáveis (Drag-and-Drop) e Limites do Viewport**: Ao implementar botões flutuantes arrastáveis, certifique-se de que seus limites de movimentação (`clamp`) considerem a barra de navegação ativa do dispositivo (ex: limite inferior de `80.0` no mobile para evitar sobrepor a `BottomNavigationBar`, e `16.0` no desktop). Adicionalmente, controle a redefinição de variáveis de posição na UI com base na mudança real do layout (`isDesktop` atualizado), impedindo que o botão dê saltos ("jumping") indesejados ao arrastar.
66. **Consistência de Status de Alarme no Calendário**: Ao carregar alarmes para o Dashboard em uma data selecionada, limpe os campos de status temporariamente (`lastStatus` e `lastStatusDate` vazios) caso a data selecionada seja futura ou se, no dia de hoje, a data do último status gravada no banco de dados for de um dia anterior. Isso garante que o usuário veja os cards no estado correto de "Pendente" em vez de carregar status defasados de dias anteriores ou exibir alarmes futuros como perdidos.
